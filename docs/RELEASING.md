...
3. Merge the reviewed commit to `main` and wait for both native CI targets to pass. Protect the release scope: no private logs, credentials, voices, local paths or user content may enter Git or an asset.
...
5. The release workflow calls the native regression workflow, then builds both archives twice and compares bytes. It checks the local package with a pinned official Store manifest/schema/API validator and publishes the add-on, source bundle, `SHA256SUMS` and `release-metadata.json` using the workflow's limited repository token. It fails on a tag/version mismatch and refuses to overwrite an existing release.
